...
function main() {
  ...
  htmlGameBanner.innerHTML = "<p id='continue'>Press any key to start!</p>";
  ...
  document.onkeyup = function(event) {
    ...
    // clear game banner if user is playing; reset user guess successful var
    if (wordOver === false) {
      ...
      if (strLettersGuessed.indexOf(strGuess.toUpperCase()) === -1) {
        ...
        arrLetterObjects.forEach(function(current, index, arr) {
          if (current.letter.toLowerCase() === strGuess.toLowerCase()) {
            ...
            document.getElementById(current.wordIndex).textContent =
              current.letter;
            ...
            if (
              ...
              htmlUserScore.textContent = "Score: " + userScore;
              htmlGameBanner.innerHTML =
                "You guessed the word <strong>" +
                strWord +
                "</strong>! <p id='continue'>Press any key to continue</p>";
        ...
        if (userGuessSuccess === false) {
          ...
          if (guessesLeft < 1) {
            wordOver = true;
            htmlGameBanner.innerHTML =
              "You ran out of guesses! <p id='continue'>Press any key to continue</p>";
            if (selectNewWord()) {
              return;
            }
          }
...
function selectNewWord() {
  ...
  if (arrWordPlayList.length < 2 || gameOver) {
    console.log("game over: " + gameOver);
    gameOver = true;
    htmlGameBanner.textContent = "Game Over!";
